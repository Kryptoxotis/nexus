...
class OrganizationRepository {
    ...
    suspend fun getOrganizations(): Result<List<Organization>> {
        return try {
            ...
            Result.Success(orgs.filter { it.id != null }.map { it.toDomain() })
        } catch (e: Exception) {
            Logger.e(TAG, "Failed to fetch organizations", e)
    ...
    suspend fun getMyOrganization(): Result<Organization?> {
        return try {
            ...
            val org = SupabaseClientProvider.getClient().postgrest["organizations"]
    ...
    suspend fun createOrganization(
        ...
        return try {
            val userId = getCurrentUserId() ?: return Result.Error("Not authenticated")
            val created = SupabaseClientProvider.getClient().postgrest["organizations"]
                .insert(OrganizationDto(
                    name = name, type = type, description = description,
                    ownerId = userId, enrollmentMode = enrollmentMode.toDbString()
                )) { select() }
    ...
    suspend fun updateOrganization(
        orgId: String, name: String? = null, description: String? = null,
        enrollmentMode: EnrollmentMode? = null, staticPin: String? = null,
        allowSelfEnrollment: Boolean? = null
    ): Result<Unit> {
        return try {
            SupabaseClientProvider.getClient().postgrest["organizations"].update({
    ...
    suspend fun createEnrollmentPin(orgId: String, pinCode: String, expiresAt: String? = null): Result<Unit> {
        return try {
            SupabaseClientProvider.getClient().postgrest["enrollment_pins"].insert(
                EnrollmentPinDto(organizationId = orgId, pinCode = pinCode, expiresAt = expiresAt)
            )
            Result.Success(Unit)
        } catch (e: Exception) {
    ...
    suspend fun validateEnrollmentPin(orgId: String, pinCode: String): Result<Boolean> {
        return try {
            val pin = SupabaseClientProvider.getClient().postgrest["enrollment_pins"]
                .select { filter { eq("organization_id", orgId); eq("pin_code", pinCode); eq("is_used", false) } }
    ...
    private fun OrganizationDto.toDomain(): Organization = Organization(
        ...
        name = name, type = type, description = description, logoUrl = logoUrl,
        ownerId = ownerId, enrollmentMode = EnrollmentMode.fromString(enrollmentMode),
        staticPin = staticPin, allowSelfEnrollment = allowSelfEnrollment,
